...
def view_patient(
    ...
    log_event(db, "DOCTOR_VIEW_PATIENT", "Patient", str(perm.patient_id), current_user.id, ip_address=ip)
    return patient
...
def view_records(
    ...
    ip = request.client.host if request.client else None
    log_event(db, "DOCTOR_VIEW_RECORDS", "MedicalRecord", None, current_user.id, ip_address=ip)
    return records
...
def view_medications(
    ...
    medications = (
        db.query(Medication)
        .filter(Medication.patient_id == perm.patient_id, Medication.is_active == True)
        .all()
    )
    ip = request.client.host if request.client else None
    log_event(db, "DOCTOR_VIEW_MEDICATIONS", "Medication", None, current_user.id, ip_address=ip)
    return medications
